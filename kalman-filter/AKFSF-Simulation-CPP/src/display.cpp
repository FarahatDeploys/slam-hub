...
void Display::saveFrame(const std::string& filename) {
    SDL_Surface* sshot = SDL_CreateRGBSurfaceWithFormat(
        0, mScreenWidth, mScreenHeight, 32, SDL_PIXELFORMAT_ARGB8888);

    SDL_RenderReadPixels(mRenderer, NULL, SDL_PIXELFORMAT_ARGB8888,
                         sshot->pixels, sshot->pitch);

    SDL_SaveBMP(sshot, filename.c_str());  // saves BMP file
